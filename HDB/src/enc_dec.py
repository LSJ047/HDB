...
#
#     img_a = (img_a.squeeze(0)).permute(1,2,0)
#     img_a = img_a.numpy()
#     # img_a = YUV2RGB(img_a)
#
...
#     savename = os.path.join(ENC_DIR,img_name)
#
#     cv2.imwrite(savename, img_a)
#
#     os.system('jpegxl/tools/cjxl "%s" %s -q 100' % (savename, os.path.join(ENC_DIR,'jpegxl.jxl'))) #ltt
#
#     filesize = os.stat(os.path.join(ENC_DIR,'jpegxl.jxl')).st_size
#
#     end_time = time()
#
...
#
#     jpegxl_bpp = 8*filesize / (H*W) # 4.619 bpp
#
...
def encode_torchac_cdf(pmf_softmax, q_res,  EMPTY_CACHE, fout):
    ...
    cdf_encode = cdf.reshape(1,c,h*w,L)
    sym_encode = sym.reshape(1,c,h*w)
    ...
    byte_stream = torchac.encode_int16_normalized_cdf(cdf_int, sym_encode.detach().cpu())
    # byte_stream = torchac.encode_float_cdf(cdf_encode.detach().cpu(), sym_encode.detach().cpu())
...
#         img_name:
#
#     Returns: 从jpegxl解码的子图像,转换为yuv X_yuv_a
#
#     '''
#
...
#
#     os.system('djxl.exe "%s" %s' % (jpegxl_name, 'output.png')) #ltt
#
#     # img_a read
...
#
#     return img_a
def decode_torchac_cdf(byte_stream,pmf_softmax,  EMPTY_CACHE):
    ...
    decoded_sym = torchac.decode_int16_normalized_cdf(cdf_int.detach().cpu(), byte_stream.read())
    ...
    decoded_sym_freq =decoded_sym.reshape(1,1,h,w)# decoded_sym_freq.index_put(decode_idx, decoded_sym)

    decoded_sym_freq = decoded_sym_freq.to(cdf_decode.device)
