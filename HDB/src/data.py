...
class ImageFolder(data.Dataset):
    ...
    def __init__(self,
                 ...
                 use_non_zeros_ac=False,
                 encode_block_size=64,
                 ac_coe_num=63,
                 ac_coe_channle_start=1,
                 use_pos_attention=False,
                 d=8
                 ) -> None:
        ...
        self.ac_coe_num = ac_coe_num
        self.ac_coe_channle_start = ac_coe_channle_start
        self.use_non_zeros_ac = use_non_zeros_ac
        self.encode_block_size = encode_block_size
        self.use_pos_attention = use_pos_attention
        self.d = d
    ...
    def to_tensor_not_normalized(self, pic: Image) -> torch.Tensor:
        ...
        if pic.mode == 'I':
            ...
            img = torch.from_numpy(np.array(pic, np.float32, copy=False))
            # img = np.array(pic, np.float32, copy=False) # ltt
        elif pic.mode == '1':
    ...
    def load(self,
        ...
        # x=self.to_tensor_not_normalized(x)
    ...
    def __getitem__(
        ...
        # img_data, shape,dct_coe_idx,non_zeros_mask,mask = self.load(io.BytesIO(file_bytes))
        # return filename, img_data, shape, dct_coe_idx,non_zeros_mask, mask
        MSB, LSB, shape = self.load(io.BytesIO(file_bytes))
        return filename, MSB, LSB, shape
